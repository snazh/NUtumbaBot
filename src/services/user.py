...
class UserService:
    ...
    async def get_profile_list_for_user(self, user_id: int):
        anketas = await self.user_repo.get_anketas_for_user(user_id)

        return anketas

    async def get_user_lovers(self, user_id: int):
        partners = await self.user_repo.get_partners(user_id)
        return partners
